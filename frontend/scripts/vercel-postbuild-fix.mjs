...
if (process.env.VERCEL && basename(projectRoot) === 'frontend') {
  ...
  const ensurePkgInParentNodeModules = (pkgPathParts) => {
    const src = resolve(projectRoot, 'node_modules', ...pkgPathParts);
    const dest = resolve(parentNodeModulesDir, ...pkgPathParts);
    if (!existsSync(src) || existsSync(dest)) return;
    mkdirSync(dirname(dest), { recursive: true });
    cpSync(src, dest, { recursive: true, force: true });
  };

  ensurePkgInParentNodeModules(['next']);
  ensurePkgInParentNodeModules(['@swc']);
  ensurePkgInParentNodeModules(['react']);
  ensurePkgInParentNodeModules(['react-dom']);
  ensurePkgInParentNodeModules(['styled-jsx']);
