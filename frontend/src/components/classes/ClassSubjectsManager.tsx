...
export function ClassSubjectsManager({
  ...
  const createMutation = useMutation({
    mutationFn: async () => {
      if (!canManageClassSubjects) {
        throw new Error(
          "Somente a Direção e a Coordenação podem vincular disciplinas às turmas.",
        );
      }
  ...
  const removeMutation = useMutation({
    mutationFn: (classSubjectId: string) => {
      if (!canManageClassSubjects) {
        throw new Error(
          "Somente a Direção e a Coordenação podem remover vínculos de disciplinas.",
        );
      }
  ...
  return (
    <>
        ...
        <div className={`${compact ? "mb-4" : "mb-6"}`}>
          ...
          <p className="text-sm text-gray-600 dark:text-gray-400">
            {canManageClassSubjects
              ? "Adicione disciplinas à turma. Para definir o professor responsável, use a distribuição na página da disciplina."
              : description}
          </p>
        </div>
      ...
      <ConfirmDialog
        ...
        onConfirm={() =>
          removingSubjectId && removeMutation.mutate(removingSubjectId)
        }
